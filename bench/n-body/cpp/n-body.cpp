// n-body — see ../c/n-body.c for the algorithm. Same scalar, fixed-order
// computation expressed with a struct array.
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
static void offset_momentum() {
    ...
    for (auto &b : bodies) { px += b.vx * b.mass; py += b.vy * b.mass; pz += b.vz * b.mass; }
...
static void advance(double dt) {
    ...
    for (auto &b : bodies) { b.x += dt * b.vx; b.y += dt * b.vy; b.z += dt * b.vz; }
}
static double energy() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int j = i + 1; j < 5; j++) {
            double dx = bodies[i].x - bodies[j].x, dy = bodies[i].y - bodies[j].y, dz = bodies[i].z - bodies[j].z;
            e -= bodies[i].mass * bodies[j].mass / std::sqrt(dx * dx + dy * dy + dz * dz);
        }
    }
    return e;
}
